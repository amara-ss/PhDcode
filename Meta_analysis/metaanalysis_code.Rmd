...
references_cleaned <- metadata_2025 %>%
...
```{r}
metadata_WOS_2025 <- metadata_WOS %>%
  dplyr::filter(Database_year=="Spain")
```
...
c <- X474_nmds_control
d <- X403_2003_10_20
e <- X403_control_2016_10_20
f <- X403_control_2015_10_20
g <- X403_control_2009_10_20
h <- X403_control_2003_10_20
```

```{r Euclidean distances}
## Dt (Distance within treatment group)
t <- rbind(
  a %>% mutate(group="2016_10_20"),
  b %>% mutate(group="2015_10_20"),
  c %>% mutate(group="2009_10_20"),
  d %>% mutate(group="2003_10_20"))
...
c <- rbind(
  e %>% mutate(group="control_2016_10_20"),
  f %>% mutate(group="control_2015_10_20"),
  g %>% mutate(group="control_2009_10_20"),
  h %>% mutate(group="control_2003_10_20")
)
