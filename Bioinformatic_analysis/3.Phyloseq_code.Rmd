...
ggplot(TOP_abundances, aes(x = stat, y = Genus, color = stat)) +
  geom_segment(aes(x = 0, xend = stat, y = Genus, yend = Genus),
               color = "grey70", linewidth = 0.8, alpha = 0.8) +
...
#ggsave("SUPP_bubbles_indspecies_TOP.png", width = 9, height = 8, dpi = 300) #### FIGURE 5a ####
...
venn.diagram(
  x = venn_list_top,
  filename = "TOPSOIL_ind_venn_diagramm.png",
  output=TRUE, 
...
#TOPSOIL
otu_cca_top <- t(as.data.frame(otu_table(ps_hel_topsoil)))
meta_cca_top <- data.frame(sample_data(ps_hel_topsoil))

meta_cca_top <- meta_cca_top %>% 
  dplyr::mutate(Fe_g_Kg = Fe_mg_Kg_105/1000,
         C_N_ratio = Ctotal_g_100g_105/Ntotal_g_100g_105)

cca_result_top <- vegan::cca(otu_cca_top ~ pH+EC+C_N_ratio+
              A_phosphorus_ppm_105+Phosphatase_activity_µmol_g_h+ 
              NH4_mgL_105+NO3_mgL_105+Ntotal_g_100g_105+
              Ctotal_g_100g_105+Carbon_stock_Mg_ha+Nitrogen_stock_Mg_ha+
              SOM_per_corregido+MBC_mg_kg_105+Nematode_g_105+Ca_g_kg+
              K_g_kg+Mg_g_kg+Na_g_kg+Al_g_kg+Fe_g_Kg+Zn_mg_Kg_105+Cu_mg_Kg_105, data= meta_cca_top)
summary(cca_result_top)
autoplot(cca_result_top, scaling="species")

#Plot:
cca_top <- fortify(cca_result_top, axes = 1:2)  
sites_top <- cca_top %>% dplyr::filter(score == "sites")
species_top <- cca_top %>% dplyr::filter(score == "species")
biplot_top <- cca_top %>% dplyr::filter(score == "biplot")

ggplot() +
  geom_point(data = sites_top, aes(x = CCA1, y = CCA2, color = meta_cca_top$TRAT2), size = 3) +
  scale_color_manual(values = cols) +
  geom_segment(data = biplot_top,
               aes(x = 0, y = 0, xend = CCA1, yend = CCA2),
               arrow = arrow(length = unit(0.25, "cm")), color = "blue") +
  geom_text(data = biplot_top,
            aes(x = CCA1, y = CCA2, label = label),
            color = "blue", vjust = -0.5, size = 3) +
  theme_minimal() +
  theme(legend.position = "none") +
  labs(x = "CCA1", y = "CCA2", color = "TRAT2")
#ggsave("cca_top.png")

#SUBSOIL
otu_cca_sub <- t(as.data.frame(otu_table(ps_hel_subsoil)))
meta_cca_sub <- data.frame(sample_data(ps_hel_subsoil))

meta_cca_sub <- meta_cca_sub %>% 
  dplyr::mutate(Fe_g_Kg = Fe_mg_Kg_105/1000,
         Ergosterol_µg_g = Ergosterol_ng_g_105/1000,
         C_N_ratio = Ctotal_g_100g_105/Ntotal_g_100g_105)

cca_result_sub <- vegan::cca(otu_cca_sub ~ pH+EC+densidad_revisada_25+C_N_ratio+
              A_phosphorus_ppm_105+Phosphatase_activity_µmol_g_h+ 
              NH4_mgL_105+NO3_mgL_105+Ergosterol_µg_g+Ntotal_g_100g_105+
              Ctotal_g_100g_105+Carbon_stock_Mg_ha+Nitrogen_stock_Mg_ha+
              SOM_per_corregido+MBC_mg_kg_105+Nematode_g_105+Ca_g_kg+
              K_g_kg+Mg_g_kg+Na_g_kg+Al_g_kg+Fe_g_Kg+Zn_mg_Kg_105+Cu_mg_Kg_105*TRAT2, data= meta_cca_sub)
summary(cca_result_sub)
autoplot(cca_result_sub, scaling="species")
```
