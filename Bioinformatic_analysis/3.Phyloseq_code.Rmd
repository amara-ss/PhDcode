...
lmm_depth <- lmer(dominance_dbp ~ TRAT2 + (1|SITE), data = alpha_fungi_SUB)
...
alpha_by_phylum_SUB <- alpha_by_phylum %>% dplyr::filter(DEPTH=="subsoil")
  ...
  lmm_asco <- lmer(dominance_dbp ~ TRAT2 + (1|SITE), data = alpha_by_phylum_TOP %>% filter(Phylum == "Ascomycota"))
...
env_top <- ggplot(pcoa_top_df_merged, aes(x = PCoA1, y = PCoA2, color = TRAT2)) +
  ...
  geom_segment(data = arrows_df,
               aes(x = 0, y = 0, xend = PCoA1*0.85, yend = PCoA2*0.85),
               arrow = arrow(length = unit(0.2, "cm")),
               size = 0.6, color = "grey30",
               inherit.aes = FALSE) +
  # Texto con nombres de variables
...
```{r}
combined_plot <- env_top + env_sub + plot_layout(ncol = 1, heights = c(2, 2))
combined_plot
#ggsave("combined_envfit2.png", width = 5, height = 8, dpi = 300)
```
...
phychem_lmTOP <- lm(observed ~ C_N_ratio + Phosphatase_activity_µmol_g_h + 
    Carbon_stock_Mg_ha + Nitrogen_stock_Mg_ha + MBC_mg_kg_105 + 
    Mg_g_kg, data = alpha_fungi_top_scaled)

phychem_lmTOP <- lm(observed ~ pH + A_phosphorus_ppm_105 + NH4_mgL_105 + 
    MBC_mg_kg_105 + K_g_kg + Mg_g_kg + Al_g_kg, data = alpha_fungi_top_scaled)
summary(phychem_lmTOP)
