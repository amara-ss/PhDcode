...
permanova <- vegan::adonis2(phyloseq::distance(ps_hel, method="bray") ~ TRAT2*DEPTH,
             permutations = 999, data = ps_hel_meta_data)
permanova

####TOPSOIL
meta_data_top <- ps_hel_meta_data %>%
  dplyr::filter(DEPTH=="topsoil")
permanova_top <- vegan::adonis2(phyloseq::distance(ps_hel_topsoil, method="bray") ~ C_N_ratio,
                                permutations = 999, data = meta_data_top)
permanova_top

####SUBSOIL
meta_data_sub <- ps_hel_meta_data %>%
  dplyr::filter(DEPTH=="subsoil")
permanova_sub <- vegan::adonis2(phyloseq::distance(ps_hel_subsoil, method="bray") ~ pH,
...
ggplot(pcoa_df, aes(x = PCoA1, y = PCoA2, color = TRAT2, shape = DEPTH)) +
  geom_point(size = 4) +
  geom_segment(data = arrows_df, aes(x = 0, y = 0, xend = PCoA1, yend = PCoA2),
               arrow = arrow(length = unit(0.3, "cm")), color = "black") +
  geom_text(data = arrows_df, aes(x = PCoA1, y = PCoA2, label = Variable),
...
ggplot(pcoa_top_df, aes(x = TRAT2, y = PCoA1, fill = TRAT2)) +
  geom_boxplot() +
...
ggplot(pcoa_sub_df, aes(x = TRAT2, y = PCoA1, fill = TRAT2)) +
  geom_boxplot() +
  geom_jitter(width = 0.1) +
  theme_minimal()
