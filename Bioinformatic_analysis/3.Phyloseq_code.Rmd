...
devtools::install_github("jslefche/piecewiseSEM@devel", force = TRUE)
...
Se recomienda hacer un gráfico de dispersión donde en el eje X esté el número de secuencias y en el eje Y el número de OTUs: si la relación es positiva, primero se deben estandarizar los datos por rarefacción antes de proseguir. Si la relación no es significativa se puede trabajar con la misma matriz.
...
alpha_fungi <- microbiome::alpha(ps1, index = c("observed", "diversity_shannon", "dominance_dbp")) #ps1: only for Fungi Kingdom
...
richness_summary <- alpha_fungi %>%
  ...
  dplyr::summarise(mean_richness = mean(dominance_dbp, na.rm = TRUE),
                   SE_richness = sd(dominance_dbp) / sqrt(n()))
...
##
alpha_fungi$SITE <- as.factor(alpha_fungi$SITE)
alpha_fungi$TRAT2 <- as.factor(alpha_fungi$TRAT2)

alpha_fungi_TOP <- alpha_fungi %>% dplyr::filter(DEPTH=="topsoil") 
alpha_fungi_SUB <- alpha_fungi %>% dplyr::filter(DEPTH=="subsoil")

lmm_depth <- lmer(observed ~ TRAT2 + (1|SITE), data = alpha_fungi_TOP)
summary(lmm_depth)

r.squaredGLMM(lmm_depth)

emm <- emmeans(lmm_depth, ~ TRAT2)
emm

par(mfrow=c(1,3))
qqnorm(resid(lmm_depth)); qqline(resid(lmm_depth))
hist(resid(lmm_depth))
plot(fitted(lmm_depth), resid(lmm_depth)); abline(h=0)
par(mfrow=c(1,1))

car::Anova(lmm_depth, test.statistic="F", type=3)

multcomp::cld(emmeans(lmm_depth, ~TRAT2, type="response"), Letters= letters) #Tuckey's post-hoc test
```
...
p$layers <- p$layers[-1]
p+  geom_boxplot(aes(fill = TRAT2), alpha=.2, size=0.3) + 
  geom_jitter(width = 0.2, size = 3, alpha=.5) +  
...
alpha_fungi_plot <- alpha_fungi %>%
  dplyr::select(TRAT2, DEPTH, observed, diversity_shannon, dominance_dbp)
# Pivot longer
alpha_fungi_long <- alpha_fungi_plot %>%
  pivot_longer(
    cols = c(dominance_dbp, diversity_shannon, observed),
    names_to = "Index",
    values_to = "Value"
  )

alpha_fungi_long$TRAT2 <- factor(alpha_fungi_long$TRAT2, levels = c("UB", "LS", "HS1", "HS2") )
alpha_fungi_long$Index <- factor(alpha_fungi_long$Index, levels = c("observed", "diversity_shannon", "dominance_dbp") )
...
ggplot(alpha_fungi_long, aes(x=TRAT2, y = Value, alpha=.6, color=TRAT2))+
  geom_boxplot(aes(fill = TRAT2), size = 0.3, alpha=.2) + 
  geom_jitter(width = 0.2, size = 3, alpha=.5) + # puntos individuales
  scale_color_manual(values = cols) + 
  scale_fill_manual(values = cols) +
  theme_bw()+
  theme(
...
alpha_asco <- microbiome::alpha(ps_asco, index = c("observed", "diversity_shannon", "dominance_dbp"))

# Basidiomycota
ps_basi <- phyloseq::subset_taxa(ps1, Phylum == "p__Basidiomycota")
alpha_basi <- microbiome::alpha(ps_basi, index = c("observed", "diversity_shannon", "dominance_dbp"))
...
alpha_phylum_plot <- alpha_by_phylum %>%
  dplyr::select(TRAT2, DEPTH, Phylum, observed, diversity_shannon, dominance_dbp)
# Pivot longer
alpha_phylum_long <- alpha_phylum_plot %>%
  pivot_longer(
    cols = c(diversity_shannon, observed, dominance_dbp),
    names_to = "Index",
    values_to = "Value"
  )

alpha_phylum_long$TRAT2 <- factor(alpha_phylum_long$TRAT2, levels = c("UB", "LS", "HS1", "HS2") )
alpha_phylum_long$Index <- factor(alpha_phylum_long$Index, levels = c("observed", "diversity_shannon", "dominance_dbp") )
alpha_phylum_long$DEPTH <- factor(alpha_phylum_long$DEPTH, levels = c("topsoil","subsoil"))

ggplot(alpha_phylum_long, aes(x=TRAT2, y = Value, shape = Phylum, alpha=.1, color=TRAT2))+
  geom_boxplot(aes(fill = TRAT2), alpha=.1) + 
  geom_jitter(width = 0.3, size = 3, alpha=.5) + 
  scale_color_manual(values = cols) + 
  scale_fill_manual(values = cols) +
  theme_bw()+
  theme(
  ...
  facet_grid(Index~DEPTH, scales="free_y")
ggsave("alpha_asco_basidio.png", width = 6, height = 7, dpi = 300)
...
alpha_by_phylum_SUB <- alpha_by_phylum %>% dplyr::filter(DEPTH=="subsoil")
  ...
  lmm_asco <- lmer(diversity_shannon ~ TRAT2 + (1|SITE), data = alpha_by_phylum_TOP %>% filter(Phylum == "Basidiomycota"))
...
ggplot(df_percent2, aes(x = TRAT2, y = Abundance, fill = Phylum)) +
  ...
  facet_wrap(~DEPTH)
ggsave("stacked_phylum5.png", width = 8, height = 5, dpi = 600) #### FIGURE 2 ####
```
...
# Create a data frame for the PCoA results
pcoa_top_df <- data.frame(PCoA1 = pcoa_top$points[, 1],
                      PCoA2 = pcoa_top$points[, 2],
                      Sample = rownames(pcoa_top$points))
class(pcoa_top_df)
...
mult <- 0.25  # para ajustar la longitud de las flechas al gráfico
arrows_df <- env_coord_sig_top %>%
  dplyr::mutate(PCoA1 = Dim1 * mult,
                PCoA2 = Dim2 * mult)

# Calculate centroids
centroids <- pcoa_top_df_merged %>%
...
env_top <- ggplot(pcoa_top_df_merged, aes(x = PCoA1, y = PCoA2, color = TRAT2)) +
  geom_point(size = 3, shape=21, alpha = 0.2) +
  #stat_ellipse(level = 0.95, linetype = 1, size = 0.1, alpha=0.5) +  # 95% confidence ellipses
  geom_point(data = centroids, 
             aes(x = centroid_x, y = centroid_y, color = TRAT2),
             size = 4, shape = 19) +                     # centroids as larger diamonds
  scale_color_manual(values = cols) +
  scale_fill_manual(values = cols) +
  theme_minimal() +
  #labs(x = "PCoA 1 (13.35%)", 
  ...
  scale_x_reverse() +  ## ---- ENVFIT VECTORS ----
  geom_segment(
    data = arrows_df,
...
pcoa_sub_df <- data.frame(
  ...
  Sample = rownames(pcoa_sub$points)
)
...
# Plot PCoA
env_sub <- ggplot(pcoa_sub_df, aes(x = PCoA1, y = PCoA2, color = TRAT2)) +
  #stat_ellipse(level = 0.95, linetype = 1, size = 0.1, alpha=.5) +  # 95% confidence ellipses
  geom_point(data = centroids, 
             aes(x = centroid_x, y = centroid_y, color = TRAT2),
             size = 4, shape=19) +       
  ...
  scale_color_manual(values = cols) +
  theme_minimal() +
  #labs(x = "PCoA 1 (6.36%)", y = "PCoA 2 (5.84%)") +
  theme(
    panel.background = element_blank(),
    panel.border = element_rect(colour = "black", fill = NA),
    panel.grid = element_blank(),
...
#ggsave("PCoA_sub_envfit2.png")
```

```{r}
combined_plot <- env_top + env_sub + plot_layout(ncol = 2, heights = c(2, 2))
combined_plot
ggsave("combined_envfit3.png", width = 7, height = 6, dpi = 300)
```
...
pairwise_top <- pairwise.adonis(
  bray_dist_top,                 # distance matrix
  meta_data_top$TRAT2,           # grouping variable
  perm = 999
)
pairwise_top


pairwise_sub <- pairwise.adonis(
  bray_dist_sub,
  meta_data_sub$TRAT2,
  perm = 999
)
...
meta_heatmap_sub <- alpha_fungi_sub[,c(2:4, 35:95)]
vars <- c("observed", "pH", "EC", "C_N_ratio",
              "A_phosphorus_ppm_105", 
              "NH4_mgL_105","NO3_mgL_105", 
              "Ntotal_g_100g_105", "Ctotal_g_100g_105", "Carbon_stock_Mg_ha",
              "Nitrogen_stock_Mg_ha", "SOM_per_corregido", "Ca_g_kg", 
              "K_g_kg","Mg_g_kg", "Na_g_kg", 
              "Al_g_kg", "Fe_g_Kg","Zn_mg_Kg_105", "Cu_mg_Kg_105") 

meta_heatmap_sub <- meta_heatmap_sub %>% 
  dplyr::select(all_of(vars))

env.scaled <- scale(meta_heatmap_sub)

ggcorr(meta_heatmap_top, method = c("everything", "spearman"))
## Correlación de SPEARMAN:
# -Las relaciones no tienen por qué ser lineales
# -Convierte los datos en rangos
# -Éstos no tienen  por qué estar normalizados

nutrient_vars <- c("C_N_ratio",
              "A_phosphorus_ppm_105", 
              "NH4_mgL_105","NO3_mgL_105", 
              "Ntotal_g_100g_105", 
              "Nitrogen_stock_Mg_ha", "Ca_g_kg", 
              "K_g_kg","Mg_g_kg", "Na_g_kg")

soil_vars <- c("pH", "EC")
carbon_vars <- c("Ctotal_g_100g_105", "Carbon_stock_Mg_ha", "SOM_per_corregido")
metal_vars <- c("Al_g_kg", "Fe_g_Kg")

pca_soil <- prcomp(alpha_fungi_top[,carbon_vars], scale=T)
summary(pca_soil)

pc1_loadings <- pca_soil$rotation[,1] #how much each original variable contributes to the first principal component (PC1)
sorted_loadings <- abs(pc1_loadings)[order(abs(pc1_loadings), decreasing = TRUE)]

# Plot customization (FIGURE 2)
alpha_fungi_top$TRAT2 <- ordered(alpha_fungi_top$TRAT2, levels=c("UB", "LS", "HS1", "HS2"))
pca_plot <- fviz_pca_biplot(pca_soil,
                ...
                pointshape = 16,
                col.ind = alpha_fungi_top$TRAT2, shape.ind = "21",
                palette = cols,
                repel = TRUE,
                addEllipses = TRUE,
...
ggplot(alpha_fungi_top, aes(x = Ctotal_g_100g_105, y = observed)) +
...
ps_hel_topsoil <- subset_samples(ps_hel, DEPTH == "topsoil")
abund_top <- as.data.frame(otu_table(ps_hel_topsoil)) 
abund_top <- t(abund_top)
abund_top <- as.data.frame(abund_top)

# Remove empty samples
keep <- rowSums(abund_top) > 0
abund_top <- abund_top[keep, ]
env.top   <- env.scaled[keep, ]

# Dist object
dist.abund = vegdist(abund_top, method = "bray")
dist.abund

#environmental vector - euclidean distance
dist.env = dist(env.top, method = "euclidean")

#Mantel test with vegan
mantel.top <- vegan::mantel(dist.abund, dist.env, method = "spearman", permutations = 9999, na.rm = TRUE)
mantel.top
#Mantel test with linkET
mantel_top <- linkET::mantel_test(
  spec = abund_top,
  env = env.top
) %>%
  mutate(
...
qcorrplot(linkET::correlate(env.top), type = "lower", diag = FALSE) +
  geom_square() +
  ...
  scale_colour_manual(
  values = c("< 0.01" = "#fd8d3c",      # very significant
             "0.01 - 0.05" = "#fd8d3c", # marginal
             ">= 0.05" = "grey")     # not significant
) +
...
ggsave("mantel_top.png", width = 6, height = 6, dpi = 300)
  

####SUBSOIL
ps_hel_subsoil <- subset_samples(ps_hel, DEPTH == "subsoil")
abund_sub <- as.data.frame(otu_table(ps_hel_subsoil)) 
abund_sub <- t(abund_sub)
abund_sub <- as.data.frame(abund_sub)

# Remove empty samples
keep <- rowSums(abund_sub) > 0
abund_sub <- abund_sub[keep, ]
env.sub   <- env.scaled[keep, ]

# Dist object
dist.abund = vegdist(abund_sub, method = "bray")

#environmental vector - euclidean distance
dist.env = dist(env.sub, method = "euclidean")

mantel.sub <- vegan::mantel(dist.abund, dist.env, method = "spearman", permutations = 9999, na.rm = TRUE)
mantel.sub

#Mantel test with linkET
mantel_sub <- linkET::mantel_test(
  spec = abund_sub,
  env = env.sub
) %>%
  mutate(
...
qcorrplot(linkET::correlate(env.sub), type = "lower", diag = FALSE) +
  geom_square() +
  geom_couple(aes(colour = pd, size = rd), 
              data = mantel_sub, 
              curvature = nice_curvature()) +
  ...
  guides(size = guide_legend(title = "Mantel's r",
         ...
         colour = guide_legend(title = "Mantel's p", 
                               ...
                               order = 1),
         fill = guide_colorbar(title = "Pearson's r", order = 3))
ggsave("mantel_sub2.png", width = 6, height = 6, dpi = 300)
```
...
ps1_genus_topsoil_rel <- transform_sample_counts(ps1_genus_topsoil, function(x) x / sum(x)) ## Relative abundance PER SAMPLE. Each sample’s abundances now sum to 1 (or 100%)
...
combined_plot <- b1 + b2 + plot_layout(ncol = 2, widths = c(2, 2), heights = c(2,1))
combined_plot
ggsave("combined_indic3.png", combined_plot, width = 10, height = 4.2, dpi = 300)
```
...
asv_counts <- sapply(venn_list_top, length)
asv_counts
total_asvs <- sum(asv_counts)
?venn.diagram

venn.diagram(
  x = venn_list_top,
  filename = "WHOLE_TOP.png",
  output=TRUE, 
...
```{r Combined RDA plots}
combined_plot <- h1 + h2 + plot_layout(ncol = 1, heights = c(3, 2))
combined_plot
#ggsave("combined_RDA.png", width = 5, height = 8, dpi = 300)
```

```{r Stepwise with soil variables}
#TOPSOIL
alpha_fungi_top <- alpha_fungi %>% filter(DEPTH=="topsoil")
rownames(alpha_fungi_top) = alpha_fungi_top$SampleID
alpha_fungi_top <- alpha_fungi_top %>% 
  dplyr::mutate(Fe_g_Kg = Fe_mg_Kg_105/1000,
         C_N_ratio = Ctotal_g_100g_105/Ntotal_g_100g_105)
#Standardize predictors: 
alpha_fungi_top_scaled <- alpha_fungi_top %>% 
  mutate(across(where(is.numeric), ~ as.numeric(scale(.))))

# Which soil properties explain variation in fungal richness in topsoil?
phychem_lmTOP <- lm(observed ~ pH+ EC+ C_N_ratio+
              A_phosphorus_ppm_105+  
              NH4_mgL_105+NO3_mgL_105+ 
              Ntotal_g_100g_105+ Ctotal_g_100g_105+ Carbon_stock_Mg_ha+
              Nitrogen_stock_Mg_ha+ SOM_per_corregido+
              Ca_g_kg+ K_g_kg+Mg_g_kg+ Na_g_kg+ 
              Al_g_kg+ Fe_g_Kg+Zn_mg_Kg_105+ Cu_mg_Kg_105, data=alpha_fungi_top_scaled)
summary(phychem_lmTOP)

car::vif(phychem_lmTOP) # VIF (Variance Inflation Factor) quantifies how much the variance of a regression coefficient is inflated due to collinearity with other predictors. Values above > 5 indicate concerning values... Variables are too correlated

step(lm(observed ~ pH+ EC+ C_N_ratio+
              A_phosphorus_ppm_105+  
              NH4_mgL_105+NO3_mgL_105+ 
              Ntotal_g_100g_105+ Ctotal_g_100g_105+ Carbon_stock_Mg_ha+
              Nitrogen_stock_Mg_ha+ SOM_per_corregido+
              Ca_g_kg+ K_g_kg+Mg_g_kg+ Na_g_kg+ 
              Al_g_kg+ Fe_g_Kg+Zn_mg_Kg_105+ Cu_mg_Kg_105, data=alpha_fungi_top_scaled), direction="both")

phychem_lmTOP <- lm(formula = observed ~ pH + A_phosphorus_ppm_105 + NH4_mgL_105 + 
    Ntotal_g_100g_105 + Ctotal_g_100g_105 + K_g_kg + Mg_g_kg + 
    Zn_mg_Kg_105, data = alpha_fungi_top_scaled)
summary(phychem_lmTOP)$coefficients
summary(phychem_lmTOP)
(summary(phychem_lmTOP)$r.squared)
car::vif(phychem_lmTOP) # now all the variables have a lw VIF

#SUBSOIL
alpha_fungi_sub <- alpha_fungi %>% filter(DEPTH=="subsoil")
rownames(alpha_fungi_sub) = alpha_fungi_sub$SampleID
alpha_fungi_sub <- alpha_fungi_sub %>% 
  dplyr::mutate(Fe_g_Kg = Fe_mg_Kg_105/1000,
         C_N_ratio = Ctotal_g_100g_105/Ntotal_g_100g_105)

#Standardize predictors: 
alpha_fungi_sub_scaled <- alpha_fungi_sub %>% 
  mutate(across(where(is.numeric), ~ as.numeric(scale(.))))

# Which soil properties explain variation in fungal richness in subsoil?
phychem_lmSUB <- lm(observed ~ pH+ EC+ C_N_ratio+
              A_phosphorus_ppm_105+ Phosphatase_activity_µmol_g_h+ 
              NH4_mgL_105+NO3_mgL_105+ 
              Ntotal_g_100g_105+ Ctotal_g_100g_105+ Carbon_stock_Mg_ha+
              Nitrogen_stock_Mg_ha+ SOM_per_corregido+ MBC_mg_kg_105+
              Nematode_g_105+ Ca_g_kg+ K_g_kg+Mg_g_kg+ Na_g_kg+ 
              Al_g_kg+ Fe_g_Kg+Zn_mg_Kg_105+ Cu_mg_Kg_105, data=alpha_fungi_sub)
summary(phychem_lmSUB)
...
step(lm(observed ~ pH+ EC+ C_N_ratio+
              ...
              Nitrogen_stock_Mg_ha+ SOM_per_corregido+ 
              Nematode_g_105+ Ca_g_kg+ K_g_kg+Mg_g_kg+ Na_g_kg+ 
              Al_g_kg+ Fe_g_Kg+Zn_mg_Kg_105+ Cu_mg_Kg_105, data=alpha_fungi_sub_scaled), direction="both")

phychem_lmSUB <- lm(formula = observed ~ EC + A_phosphorus_ppm_105 + NH4_mgL_105 + 
    Ctotal_g_100g_105 + Mg_g_kg + Zn_mg_Kg_105, data = alpha_fungi_sub_scaled)
summary(phychem_lmSUB)
(summary(phychem_lmSUB)$r.squared)
car::vif(phychem_lmSUB)
```

### The only way to test *causality* are SEM:
...
library(semEff)
sem_top_all <- piecewiseSEM::psem(
  lm(observed ~ pc1 + NH4_mgL_105 + Ctotal_g_100g_105 + 
       K_g_kg + Mg_g_kg + Al_g_kg, data = alpha_fungi_top),
  #lm(observed ~pc1, data = alpha_fungi_top),
  lm(NH4_mgL_105 ~pc1, data = alpha_fungi_top),
  Ctotal_g_100g_105%~~%NH4_mgL_105,
  K_g_kg%~~%NH4_mgL_105)
summary(sem_top_all, .progressBar=F, standardize="range",conserve=TRUE)
write.csv(d$coefficients, "SEM_topsoil_all.csv")

# Look at indirect/total effects for significant paths
# Have to turn psem into list first for semEFF package
semmodel_list <- list(
  lm(observed ~ pc1 + NH4_mgL_105 + Ctotal_g_100g_105 + K_g_kg + Mg_g_kg + Al_g_kg, data = alpha_fungi_top),
  lm(NH4_mgL_105 ~ pc1, data = alpha_fungi_top)   # mediator path
)
# Bootstrapping to calculate indirect effects
# R = number of resamples (10,000 recommended, can reduce to 1,000 for testing)
semmodel_boot <- bootEff(semmodel_list, R = 1000, parallel = "no", incl.raw = TRUE)
semmodel_eff <- semEff(semmodel_boot, use.raw = TRUE)
summary(semmodel_eff, response = "observed")


#Subsoil
library(piecewiseSEM)
sem_sub_all <- piecewiseSEM::psem(
  lm(observed ~ pc1+ A_phosphorus_ppm_105 + NH4_mgL_105 + 
    Ctotal_g_100g_105 + Mg_g_kg + Zn_mg_Kg_105, data = alpha_fungi_SUB),
  lm(NH4_mgL_105 ~pc1, data = alpha_fungi_SUB),
  Ctotal_g_100g_105%~~%NH4_mgL_105,
  NH4_mgL_105%~~%Mg_g_kg
)
summary(sem_sub_all, .progressBar=F, standardize="range",conserve=TRUE)
#write.csv(c$coefficients, "SEM_subsoil_all.csv")

# Look at indirect/total effects for significant paths
# Have to turn psem into list first for semEFF package
semmodel_list_sub <- list(
  lm(observed ~ A_phosphorus_ppm_105 + NH4_mgL_105 + 
    Ctotal_g_100g_105 + Mg_g_kg + Zn_mg_Kg_105, data = alpha_fungi_SUB),
  lm(NH4_mgL_105 ~pc1, data = alpha_fungi_SUB)   # mediator path
)
# Bootstrapping to calculate indirect effects
# R = number of resamples (10,000 recommended, can reduce to 1,000 for testing)
semmodel_boot <- bootEff(semmodel_list_sub, R = 1000, parallel = "no", incl.raw = TRUE)
semmodel_eff <- semEff(semmodel_boot, use.raw = TRUE)
summary(semmodel_eff, response = "observed")

################################### GUILDS ##############################################
## We want to know what influences most ectomycorrhizal richness vs saprotrophs richness 
SEM_df_top <- guild_richness_SEM %>% dplyr::filter(DEPTH=="topsoil")
SEM_df_sub <- guild_richness_SEM %>% dplyr::filter(DEPTH=="subsoil")

# Topsoil
library(piecewiseSEM)
sem_top <- piecewiseSEM::psem(
  lm(ECM_richness ~ pc1 + A_phosphorus_ppm_105 + NH4_mgL_105 + 
    Ntotal_g_100g_105 + Ctotal_g_100g_105 + K_g_kg +  
    Zn_mg_Kg_105, data = SEM_df_top),
  lm(LSp_richness ~ pc1 + A_phosphorus_ppm_105 + NH4_mgL_105 + 
    Ntotal_g_100g_105 + Ctotal_g_100g_105 + K_g_kg + 
    Zn_mg_Kg_105, data = SEM_df_top),
  lm(USp_richness ~ pc1 + A_phosphorus_ppm_105 + NH4_mgL_105 + 
    Ntotal_g_100g_105 + Ctotal_g_100g_105 + K_g_kg + 
      SOM_per_corregido, data = SEM_df_top),
  
  Ntotal_g_100g_105%~~%Ctotal_g_100g_105
)
summary(sem_top, .progressBar=F, standardize="range",conserve=TRUE)
#write.csv(a$coefficients, "SEM_topsoil.csv")

# Look at indirect/total effects for significant paths
# Have to turn psem into list first for semEFF package
semmodel_list_guildtop <- list(
  lm(ECM_richness ~ pc1 + A_phosphorus_ppm_105 + NH4_mgL_105 + 
    Ntotal_g_100g_105 + Ctotal_g_100g_105 + K_g_kg +  
    Zn_mg_Kg_105, data = SEM_df_top),
  lm(LSp_richness ~ pc1 + A_phosphorus_ppm_105 + NH4_mgL_105 + 
    Ntotal_g_100g_105 + Ctotal_g_100g_105 + K_g_kg + 
    Zn_mg_Kg_105 +ECM_richness, data = SEM_df_top),
  lm(USp_richness ~ pc1 + A_phosphorus_ppm_105 + NH4_mgL_105 + 
    Ntotal_g_100g_105 + Ctotal_g_100g_105 + K_g_kg + 
      SOM_per_corregido + LSp_richness, data = SEM_df_top)
)
# Bootstrapping to calculate indirect effects
# R = number of resamples (10,000 recommended, can reduce to 1,000 for testing)
semmodel_boot <- bootEff(semmodel_list_guildtop, R = 1000, parallel = "no", incl.raw = TRUE)
semmodel_eff <- semEff(semmodel_boot, use.raw = TRUE)
summary(semmodel_eff, response = "USp.richness")

#Subsoil
library(piecewiseSEM)
sem_sub <- piecewiseSEM::psem(
  lm(ECM_richness ~ pc1 + C_N_ratio + A_phosphorus_ppm_105 + 
    Ctotal_g_100g_105 + NH4_mgL_105 + 
    SOM_per_corregido + Ca_g_kg + Na_g_kg + Al_g_kg, data = SEM_df_sub),
  lm(C_N_ratio ~pc1, data = SEM_df_sub),
  #lm( ~pc1, data = SEM_df_sub),
  lm(LSp_richness ~ pc1 + C_N_ratio + A_phosphorus_ppm_105 + 
    Ctotal_g_100g_105 + NH4_mgL_105 + 
    SOM_per_corregido + Ca_g_kg + Na_g_kg + Al_g_kg, data = SEM_df_sub),
  lm(USp_richness ~ pc1 + C_N_ratio + A_phosphorus_ppm_105 + 
    NO3_mgL_105 + Ctotal_g_100g_105 + NH4_mgL_105 + 
    Ca_g_kg + Na_g_kg + Al_g_kg, data = SEM_df_sub),
  Ntotal_g_100g_105%~~%Ctotal_g_100g_105
)
summary(sem_sub, .progressBar=F, standardize="range",conserve=TRUE)
#write.csv(b$coefficients, "SEM_subsoil.csv")

# Look at indirect/total effects for significant paths
# Have to turn psem into list first for semEFF package
semmodel_list_guildsub <- list(
  lm(ECM_richness ~ pc1 + C_N_ratio + A_phosphorus_ppm_105 + 
    NO3_mgL_105 + Ctotal_g_100g_105 + NH4_mgL_105 + 
    SOM_per_corregido + Ca_g_kg + Na_g_kg + Al_g_kg, data = SEM_df_sub),
  lm(C_N_ratio ~pc1, data = SEM_df_sub),
  lm(NO3_mgL_105 ~pc1, data = SEM_df_sub),
  lm(LSp_richness ~ pc1 + C_N_ratio + A_phosphorus_ppm_105 + 
    NO3_mgL_105 + Ctotal_g_100g_105 + NH4_mgL_105 + 
    SOM_per_corregido + Ca_g_kg + Na_g_kg + Al_g_kg, data = SEM_df_sub),
  lm(USp_richness ~ pc1 + C_N_ratio + A_phosphorus_ppm_105 + 
...
# Bootstrapping to calculate indirect effects
# R = number of resamples (10,000 recommended, can reduce to 1,000 for testing)
semmodel_boot <- bootEff(semmodel_list_guildsub, R = 1000, parallel = "no", incl.raw = TRUE)
semmodel_eff <- semEff(semmodel_boot, use.raw = TRUE)
summary(semmodel_eff, response = "ECM_richness")
```
...
for_RF <- for_RF %>% filter(DEPTH=="topsoil")
...
RF_functions_calc <- for_RF %>%
  ...
  dplyr::mutate(
    ...
    Nutrient_cycling = mean(c(Phosphatase_activity_µmol_g_h, MBC_mg_kg_105), na.rm = TRUE),
