...
# 4️⃣ Plot tree with ggtree
ggtree(tree, layout = "circular", open.angle = 30) %<+% indicator_df +
  #geom_tiplab(size=2) +
  geom_tippoint(aes(color = TRAT2), size = 3) +
  #scale_color_manual(values = cols)+
  theme_tree2() +
  theme(legend.position = "right") +
  scale_color_brewer(palette = "Set1")
#ggsave("indic_phy_SUB.png", width = 10, height = 10, dpi = 300)
```
...
# OTU table con abundancia de tus indicadores por muestra
otu_ind <- otu_table(ps1_sub)[ind_spp, , drop=FALSE]

# matriz binaria de presencia/ausencia por tratamiento
# por ejemplo, ASVs presentes en cada tratamiento
trait_matrix <- indicator_df %>%
  dplyr::mutate(value = 1) %>%
  pivot_wider(names_from = TRAT2, values_from = value, values_fill = 0) %>%
  column_to_rownames("ASV")

# Filogenetic clustering: calcula MPD por grupo
ses.mpd_result <- ses.mpd(samp = trait_matrix, dis = ordu_unw, null.model = "taxa.labels")
?ses.mpd
```
