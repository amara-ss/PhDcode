...
vista <- guild_richness_summary %>% dplyr::filter(primary_lifestyle=="unspecified_saprotroph")
...
glm_results <- glm(
      Richness ~ TRAT2,
      family = poisson,
      data = filter(richness_per_sampleTOP, primary_lifestyle == "unspecified_saprotroph"))
...
### PLOT
selected_guilds$TRAT2 <- ordered(selected_guilds$TRAT2, levels=c("UB", "LS", "HS1", "HS2"))
selected_guilds$DEPTH <- ordered(selected_guilds$DEPTH, levels=c("topsoil", "subsoil"))

selected_guilds <- guild_richness_summary %>%
  filter(primary_lifestyle %in% c("ectomycorrhizal", "unspecified_saprotroph", "soil_saprotroph", "litter_saprotroph"))
  
ggplot(selected_guilds, aes(x = TRAT2, y = MeanRichness, color = primary_lifestyle, group = primary_lifestyle)) +
  geom_point(size = 3.5, alpha=.6, position = position_dodge(width = 0.3)) +
  scale_color_manual(values = c("#DD1C77","#8c6bb1","#6baed6", "#0570b0")) +
  geom_line(position = position_dodge(width = 0.3)) +
  geom_errorbar(aes(ymin = MeanRichness - SDRichness, ymax = MeanRichness + SDRichness),
                width = 0.1, position = position_dodge(width = 0.3)) +
  facet_wrap(~DEPTH) +
  labs(x = "Fire severity", y = "Mean ASV richness", color = "Guild") +
  theme_minimal()+
    theme(
...
ggplot(guild_rel_summary2, aes(x = TRAT2, y = graph_abun, fill = primary_lifestyle)) +
...
lm_results <- lm(RelAbund ~ TRAT2, data = guild_rel_TOP %>% dplyr::filter(primary_lifestyle=="unspecified_saprotroph")) 
anova(lm_results)
