...
####
soil_functions_clean <- soil_functions %>%
dplyr::select(ID,TRAT2, DEPTH, 53:64) 

# 3) Reshape the data from wide to long format
soil_long <- soil_functions_clean %>%
  ...
  dplyr::select(ID, TRAT2, DEPTH, Soil_function, Mean)
  
soil_long_se <- soil_functions_clean %>%
  ...
  dplyr::select(ID, TRAT2, DEPTH,SE)
...
#ggsave("output/soil_functions_vertical.png", width = 3, height = 7, dpi = 300)

##### Linear models
soil_functions_clean_sub <- soil_functions_clean %>%
  filter(DEPTH=="topsoil")
var_func <- soil_functions_clean_sub$Soil_activity

lm_f <- aov(var_func ~ TRAT2, data = soil_functions_clean_sub)
summary(lm_f)
anova(lm_f)
Tuckeys<- TukeyHSD(lm_f)
...
selected_vars <- names(depth_dif) %>% 
  intersect(c("pH_difference", "EC_difference", "A_phosphorus_ppm_105_difference", 
              "NO3_mgL_105_difference", "Phosphatase_activity_µmol_g_h_difference", 
              "Ergosterol_µg_g_difference", "SOM_per_corregido_difference",
              "Nematode_g_105_difference"))
...
depth_dif %>%
  ...
  facet_wrap(vars(forcats::fct_relevel(name, "pH_difference", 
                                       "A_phosphorus_ppm_105_difference", 
                                       "NO3_mgL_105_difference", "EC_difference", 
                                       "Phosphatase_activity_µmol_g_h_difference",
                                       "Ergosterol_µg_g_difference", 
                                       "SOM_per_corregido_difference",
                                       "Nematode_g_105_difference",
