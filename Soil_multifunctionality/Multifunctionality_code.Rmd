...
dataset_simple <- dataset %>%
  dplyr::select(ID, TRAT2, DEPTH, all_of(resp_variables))
...
var <- dataset_pc1$C_N_ratio
var_log <- (log(dataset_pc1$C_N_ratio)+1)
...
##############################################################
mixed.var <- lmer(var_log ~ TRAT2*DEPTH + (1|SITE), data = dataset_pc1)
...
var_top <- dataset_topsoil$SOM_per_corregido
var_log_top <- (log(dataset_topsoil$C_N_ratio)+1)

hist(var_log_top)

## PC1 as fixed effect

mixed.pc1_top <- lmer(var_top ~ pc1_top + (1|SITE), data = dataset_topsoil)
...
mixed.TRAT2_05 <- lmer(var_log_top ~ TRAT2 + (1|SITE), data = dataset_topsoil)
...
var_sub <- dataset_subsoil$SOM_per_corregido
var_log_sub <- (log(dataset_subsoil$C_N_ratio)+1)

hist(var_log_sub)

## PC1 as fixed effect

mixed.pc1_sub <- lmer(var_log_sub ~ pc1_sub + (1|SITE), data = dataset_subsoil)
...
mixed.TRAT2_510 <- lmer(var_log_sub ~ TRAT2 + (1|SITE), data = dataset_subsoil)
...
soil_functions <- std_response %>%
  ...
  dplyr::mutate(
    ...
    Nutrient_pools = mean(c(A_phosphorus_ppm_105_standardized, NH4_mgL_105_standardized, NO3_mgL_105_standardized, Ntotal_g_100g_105_standardized, Nitrogen_stock_Mg_ha_standardized, C_N_ratio_standardized, Ca_g_kg_standardized, K_g_kg_standardized, Mg_g_kg_standardized, Na_g_kg_standardized), na.rm = TRUE),
    Soil_carbon = mean(c(Ctotal_g_100g_105_standardized, SOM_per_corregido_standardized, Carbon_stock_Mg_ha_standardized), na.rm = TRUE),
    Soil_activity = mean(c(Phosphatase_activity_µmol_g_h_standardized, MBC_mg_kg_105_standardized), na.rm = TRUE),
    Heavy_metals = mean(c(Al_g_kg_standardized, Cu_mg_Kg_105_standardized, Fe_g_Kg_standardized, Zn_mg_Kg_105_standardized), na.rm = TRUE),
    Soil_biota = mean(c(Nematode_g_105_standardized, Ergosterol_µg_g_standardized), na.rm = TRUE))
...
dplyr::summarise(
        ...
        SE_nutrient_pools = se(Nutrient_pools), 
        Nutrient_pools = mean(Nutrient_pools),
        SE_soil_carbon = se(Soil_carbon), 
        Soil_carbon = mean(Soil_carbon),
        SE_soil_activity = se(Soil_activity), 
        Soil_activity = mean(Soil_activity),
        SE_heavy_metals = se(Heavy_metals), 
        Heavy_metals = mean(Heavy_metals),
        SE_soil_biota = se(Soil_biota),
        Soil_biota = mean(Soil_biota)
        )
...
soil_long$Soil_function <- factor(soil_long$Soil_function, 
                           levels = c("Soil_properties", "Nutrient_pools",
                                      "Soil_carbon", "Soil_activity",
                                      "Soil_biota", "Heavy_metals"))
...
ggplot(soil_long, aes(x = TRAT2, y = Mean, fill = DEPTH)) +
...
#ggsave("output/soil_functions_long.png", width = 3, height = 7, dpi = 300)

##### Linear models
soil_functions_sub <- soil_functions %>%
  filter(DEPTH=="topsoil")

var_func <- soil_functions_sub$Heavy_metals

lm_f <- aov(var_func ~ TRAT2, data = soil_functions_sub)
summary(lm_f)
anova(lm_f)
Tuckeys<- TukeyHSD(lm_f)
...
```{r Ordination matrix}
#These analysis are sensitive to the scale of the different response variables so the first thing we do is to standardize the response variables if they are not in the same units:
...
NMDS0<-metaMDS(ordination_dist, distance="euclidean, k=2")
summary(NMDS0)
NMDS0$stress

NMDS0.perm <- adonis2(ordination_dist~ ordination_df$TRAT2*ordination_df$DEPTH) 
NMDS0.perm # P = .001

nem_pairwise=pairwise.adonis2(ordination_dist ~  TRAT2*DEPTH, data = ordination_df)
nem_pairwise
```
...
#Topsoil:
set.seed(123)
nmds_top<-metaMDS(ordination_df_topsoil, k=2)
nmds_top

variables_top <- as.data.frame(scores(nmds_top, "species"))
variables_top

coordenadas_top <- as.data.frame(scores(nmds_top)$sites) #The scores represent samples in the new ordination space.
coordenadas_top

coordenadas_top$TRAT2 = dataset_topsoil$TRAT2 
coordenadas_top$ID = dataset_topsoil$ID
coordenadas_top$DEPTH = dataset_topsoil$DEPTH
head(coordenadas_top)

#Add enviromental data
env_df <- dataset_topsoil %>%
  dplyr::select(all_of(plot_vars))

env_nmds <- envfit(nmds_top, env_df, permutations = 999, na.rm = TRUE)
env_nmds

env_coord <- as.data.frame(scores(env_nmds, "vectors"))
env_coord_sig <- scores(env_nmds, "vectors")[env_nmds$vectors$pvals < 0.05, ] #Significant environmental vectors

#############################################

#Subsoil:
set.seed(123)
nmds<-metaMDS(ordination_df_subsoil, k=2)
nmds

variables_sub <- as.data.frame(scores(nmds, "species"))
variables_sub

coordenadas_sub <- as.data.frame(scores(nmds)$sites) #The scores represent samples in the new ordination space.
coordenadas_sub

coordenadas_sub$TRAT2 = dataset_subsoil$TRAT2 
coordenadas_sub$ID = dataset_subsoil$ID
coordenadas_sub$DEPTH = dataset_subsoil$DEPTH
head(coordenadas_sub)

#Add enviromental data
env_df <- dataset_subsoil %>%
  dplyr::select(all_of(plot_vars))
...
```{r NMDS: visualization of centroids}
#Topsoil
metadata_nmds_top <- inner_join(dataset_simple, coordenadas_top, by=c("ID","TRAT2", "DEPTH"))
metadata_nmds_top
...
star <- metadata_nmds_top %>%
  filter(DEPTH == "topsoil") %>%  # Filter topsoil data
...
star$TRAT2 <- ordered(star$TRAT2, levels=c("UB", "LS", "HS1", "HS2"))
...
ggplot(star, aes(x=NMDS1, xend=centroid1, 
  ...
  geom_point(data=centroid_top, mapping = aes(x=axis1, y=axis2, color=TRAT2), shape=19, alpha=0.9, size=5, inherit.aes = FALSE) +
...
#ggsave("output/nmds_coords_top.png")

#################################################

#Subsoil
metadata_nmds_sub <- inner_join(dataset_simple, coordenadas_sub, by=c("ID","TRAT2", "DEPTH"))
metadata_nmds_sub
...
ggplot(star_sub, aes(x=NMDS1, xend=centroid1, 
  ...
  #geom_text(data = env_coord_sig, aes(x = 2*NMDS1, y = 2*NMDS2), colour = "grey30", 
  #          label = row.names(env_coord_sig), inherit.aes = FALSE) + 
  geom_point(data=centroid_sub, mapping = aes(x=axis1, y=axis2, color=TRAT2), shape=19, alpha=0.9, size=5, inherit.aes = FALSE) +
  scale_color_manual(values=cols) +
  theme_minimal()+
  theme(
    panel.background = element_blank(),
...
#ggsave("output/nmds_coords_sub.png")
```

```{r NMDS: ellipses}
ggplot(metadata_nmds_top, aes(x=NMDS1, y=NMDS2, color=TRAT2, fill=TRAT2)) +
  geom_point(size=4, alpha=0.5) +
  geom_point(data=centroid_top, mapping = aes(x=axis1, y=axis2, color=TRAT2), shape=15, size=4.5, inherit.aes = FALSE) +
...
To test the similarity of the response variables within the same fire severity class and between different severity classes, we will perform an *ANOSIM*:
- *Non-parametric*
- Does NOT account for interactions 
- It *ranks* the values in the resemblance matrix before proceeding with the analysis
- The information of interest is the *relationships among the dissimilarities* (it will work with the ranks on the transformed matrix)
- *Statistical significance* means that the response variables are very similar within groups and different among groups
- When the variance within a group is considerable, this will overlap and the difference among groups will not be significant
- The outcome will be an *R statistic* ranging from -1 to 1 and a significance value
- Recommended for NMDS

```{r ANOSIM - Analysis of Similarities}
#Whole data:
anosim <- vegan::anosim(ordination_df0, TRAT2, distance="euclidean", permutations=9999)

#Topsoil:
anosim_topsoil <-vegan::anosim(ordination_df_topsoil, dataset_topsoil$TRAT2, distance="euclidean", permutations=9999)

#Subsoil
anosim_subsoil<-vegan::anosim(ordination_df_subsoil, dataset_subsoil$TRAT2, distance="euclidean", permutations=9999)

#Results
anosim
anosim_topsoil #R-values close to 1 mean that our groups are extremely different
anosim_subsoil #R-values close to 1 mean that our groups are extremely different
```

*PERMANOVA* will work with the *raw* numbers in the dissimilarities matrix:
- *Parametric*
- It DOES account for interactions
- The outcome is a *Fisher's F*
- Recommended for PCA or PCoA

```{r PERMANOVA}
# Run the adonis2 analysis
p <- adonis2(dataset[,plot_vars] ~ TRAT2, data = dataset, permutations = 9999)
p
```

```{r}
h <- adonis2(ordination_df0~TRAT2*DEPTH, ordination_df, distance="euclidean")
h
adonis_results <- as.data.frame(h)
adonis_results
```
...
duffyAllVars.std <- paste0(resp_variables, ".std")
...
asduffy %>%
  ...
  scale_x_reverse()
#ggsave("output/multifunc_plot.png")
```
...
sig_vars <- names(df) %>% 
  intersect(c("EC", "A_phosphorus_ppm_105", "Phosphatase_activity_µmol_g_h", 
              "NH4_mgL_105","NO3_mgL_105", "Ergosterol_µg_g", "Ctotal_g_100g_105",
              "Carbon_stock_t_ha", "SOM_per_corregido", "MBC_mg_kg_105", 
              "Nematode_g_105", "Al_g_kg", "Fe_g_Kg", "Cu_mg_Kg_105"))
...
Firefungi_simple %>%
  ...
  facet_wrap(vars(forcats::fct_relevel(name, "EC", "A_phosphorus_ppm_105",
                                       "NH4_mgL_105", "NO3_mgL_105", "Ctotal_g_100g_105",
                                       "SOM_per_corregido","Carbon_stock_Mg_ha",
                                       "Phosphatase_activity_µmol_g_h", "MBC_mg_kg_105",
                                       "Ergosterol_µg_g", "Nematode_g_105",
                                       "Al_g_kg", "Fe_g_Kg","Cu_mg_Kg_105")), 
...
#ggsave("output/depth_dif.png", height = 7, width = 6.5, dpi = 300)

##### Linear models
var_dif <- depth_dif$Bulk_density_g_cm3_difference
