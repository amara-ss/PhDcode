...
pacman::p_load("ggplot2","tidyr","dplyr","tidyverse","readxl","pastecs","lattice","forcats","corrgram","corrplot","HH","effects","car","multcompView","lme4","emmeans","factoextra","ggfortify","RColorBrewer","MuMIn","scales","multifunc", "ggthemes", "knitr", "vegan", "hrbrthemes", "indicspecies")
...
```{r Read data}
Firefungi <- read_xlsx(paste0(dir,"/0_FireFungi_MyDataset_copia.xlsx"))
Firefungi$TRAT2 <- ordered(Firefungi$TRAT2, levels=c("UB", "LS", "HS1", "HS2"))
...
Firefungi_topsoil <- Firefungi %>%  filter(DEPTH=="topsoil")
Firefungi_subsoil <- Firefungi %>% filter(DEPTH=="subsoil")
```

```{r Correlation Matrix}
vars_cor <- Firefungi[,c(31:51)]
ggcorr(vars_cor, method = c("everything", "pearson"))
#ggsave("vars_cor.png")
```

```{r Descriptive table of vars}
descrip <- function(Firefungi) {
  result <- Firefungi %>%
    group_by(TRAT2, DEPTH) %>%
    summarize(across(11:89, list(
      mean = ~mean(.),
      se = ~sd(.) / sqrt(n())
    ), .names = "{col}_{fn}"))
...
```{r PCA with plot characterizarion vars}
pca_burn_sev <- prcomp(Firefungi[,c(11,19:29)], scale=T)
...
## PCA devided by depths
pca_burn_sev_top <- prcomp(Firefungi_topsoil[,c(11,19:29)], scale=T)
pc1_top <- pca_burn_sev_top$x[,1] 

pca_burn_sev_sub <- prcomp(Firefungi_subsoil[,c(11,19:29)], scale=T)
