...
env_top <- envfit(nmds_top, env_df, permutations = 999, na.rm = TRUE)

env_coord <- as.data.frame(scores(env_top, "vectors"))
env_coord_sig <- scores(env_top, "vectors")[env_top$vectors$pvals < 0.05, ] #Significant environmental vectors
...
#ggsave("output/nmds_top.png", height = 5, width = 7, dpi = 300)
...
#ggsave("output/nmds_coords_top.png", height = 5, width = 7, dpi = 300)
```
...
env_sub <- envfit(nmds_sub, env_df, permutations = 999, na.rm = TRUE)

env_coord <- as.data.frame(scores(env_sub, "vectors"))
env_coord_sig <- scores(env_sub, "vectors")[env_sub$vectors$pvals < 0.05, ] #Significant environmental vectors
...
#ggsave("output/nmds_sub.png", height = 5, width = 7, dpi = 300)
...
ggsave("output/nmds_coords_sub.png", height = 5, width = 7, dpi = 300)
```
...
sig_vars <- names(dataset) %>% 
  intersect(c("EC", "Bulk_density_g_cm3","A_phosphorus_ppm_105", 
              "Phosphatase_activity_µmol_g_h", "Ntotal_g_100g_105", "C_N_ratio",
              "NH4_mgL_105","NO3_mgL_105", "Na_g_kg", "SOM_per_corregido",
              "Ergosterol_µg_g", "Carbon_stock_Mg_ha", "MBC_mg_kg_105", 
...
dataset %>%
  ...
  facet_wrap(vars(forcats::fct_relevel(name, "EC",
              "Bulk_density_g_cm3","A_phosphorus_ppm_105", 
              "Phosphatase_activity_µmol_g_h", "Ntotal_g_100g_105", "C_N_ratio",
              "NH4_mgL_105","NO3_mgL_105", "Na_g_kg", "SOM_per_corregido",
              "Ergosterol_µg_g", "Carbon_stock_Mg_ha", "MBC_mg_kg_105", 
              "Nematode_g_105", "Al_g_kg", "Fe_g_Kg", "Cu_mg_Kg_105")), 
...
#ggsave("output/sig_variables.png")
```
