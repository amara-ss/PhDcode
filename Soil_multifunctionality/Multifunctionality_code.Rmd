...
pacman::p_load("ggplot2","tidyr","dplyr","tidyverse","readxl","pastecs","lattice","forcats","corrgram","corrplot","HH","effects","car","multcompView","lme4", "lmerTest", "emmeans","factoextra","ggfortify","RColorBrewer","MuMIn","scales","multifunc", "ggthemes", "knitr", "vegan", "hrbrthemes", "indicspecies", "randomForest", "rfPermute", "devtools", "corrgram")
...
```{r Read data}
dataset <- Firefungi_soil_dataset_2021 #Updated bulk_density data 04/02/2025
#summary(dataset)

# Northness and Eastness
aspect_rad <- dataset$Aspect_degrees * pi / 180
...
as.numeric(dataset$Northness)
as.numeric(dataset$Eastness)
...
descrip %>%
  dplyr::select(TRAT2, DEPTH, starts_with("Elevation"), starts_with("Slope"))
...
##### Linear models
var <- dataset$Eastness

lm <- aov(var ~ TRAT2, data = dataset)
summary(lm)
anova(lm)
Tuckeys<- TukeyHSD(lm)
...
resp_variables <- names(dataset) %>% 
  intersect(c("pH", "EC", "densidad_revisada_25", "C_N_ratio",
...
var_log <- (log(dataset$Northness)+1)
...
```
 ...
 - *Log-transformed variables to fit the models*: EC, NH4, NO3, Ca, Mg, Total C, Carbon stock, Nitrogen stock, MBC, Ergosterol, C/N ratio, densidad_revisada_25
...
##############################################################
mixed.var <- lmer(var ~ TRAT2 + (1|SITE), data = dataset)
...
as.data.frame(VarCorr(mixed.var))
#Marginal and conditional R^2
performance::r2(mixed.var)
```

```{r Linear Mixed Models by depth: TOPSOIL}
models_topsoil <- dataset_pc1 %>% filter(DEPTH=="topsoil")
var_top <- models_topsoil$C_N_ratio
var_log_top <- (log(models_topsoil$C_N_ratio)+1)
...
#Significance of the random effect
as.data.frame(VarCorr(mixed.TRAT2_05))
#Marginal and conditional R^2
performance::r2(mixed.TRAT2_05)
```

```{r Linear Mixed Models by depth: SUBSOIL}
models_subsoil <- dataset_pc1 %>% filter(DEPTH=="subsoil")
var_sub <- models_subsoil$Cu_mg_Kg_105
var_log_sub <- (log(models_subsoil$densidad_revisada_25)+1)
...
#Significance of the random effect
as.data.frame(VarCorr(mixed.TRAT2_510))
#Marginal and conditional R^2
performance::r2(mixed.TRAT2_510)
```

```{r t-test HS1 vs HS2}
severity_data <- soil_functions %>%
  filter(TRAT2 %in% c("HS1", "HS2"))

var <- severity_data$Metals
...
std_response <- dataset_simple %>%
  dplyr::mutate(across(all_of(resp_variables),           
                ~ rescale(., to=c(0,1)),            
...
soil_functions <- std_response %>%
  ...
  dplyr::mutate(
    Soil_properties = mean(c(pH_standardized, EC_standardized, densidad_revisada_25_standardized), na.rm = TRUE),
...
#ggsave("output/Figure4_color3.png", width = 3, height = 7, dpi = 300)


## To assign statistic significance, we first run models for topsoil and subsoil separately (we want to see the differences between TRAT2 within each depth). Afterwards, we run a whole model to test the differences across depths.

##### Linear models_TOPSOIL
soil_functions_top <- soil_functions %>%
  filter(DEPTH=="topsoil")

lmf_top <- lmer(Carbon_sequestration ~ TRAT2 + (1|SITE), data = soil_functions_top)

par(mfrow=c(1,3))
qqnorm(resid(lmf_top)); qqline(resid(lmf_top))
hist(resid(lmf_top))
plot(fitted(lmf_top), resid(lmf_top)); abline(h=0)
par(mfrow=c(1,1))

posthoc <- glht(lmf_top, linfct = mcp(TRAT2 = "Tukey"))
summary(posthoc)
letters <- cld(posthoc, decreasing = TRUE)  # 'cld()' assigns letters
print(letters)

##### Linear models_SUBSOIL
soil_functions_sub <- soil_functions %>%
  filter(DEPTH=="subsoil")

lmf_sub <- lmer(Carbon_sequestration ~ TRAT2 + (1|SITE), data = soil_functions_sub)

par(mfrow=c(1,3))
qqnorm(resid(lmf_sub)); qqline(resid(lmf_sub))
hist(resid(lmf_sub))
plot(fitted(lmf_sub), resid(lmf_sub)); abline(h=0)
par(mfrow=c(1,1))

posthoc <- glht(lmf_sub, linfct = mcp(TRAT2 = "Tukey"))
summary(posthoc)
letters <- cld(posthoc, decreasing = TRUE)  # 'cld()' assigns letters
print(letters)

##### Linear models_WHOLE
lmf <- lmer(Carbon_sequestration ~ TRAT2*DEPTH + (1|SITE), data = soil_functions)

par(mfrow=c(1,3))
qqnorm(resid(lmf)); qqline(resid(lmf))
hist(resid(lmf))
plot(fitted(lmf), resid(lmf)); abline(h=0)
par(mfrow=c(1,1))

car::Anova(lmf, test.statistic="F", type=3)

multcomp::cld(emmeans(lmf, ~DEPTH*TRAT2, type="response"), Letters=letters) #Tuckey's post-hoc test
```
...
AIC <- AIC(lm.multifunc1, lm.multifunc2, lm.multifunc3, lm.multifunc4, lm.multifunc5, lm.multifunc6)
AIC

summary(lm.multifunc4) #To access the Adjusted R-squared and the p-value
anova(lm.multifunc4)

#To get the letters of the Tuckey test for this model, we need to change the ~lm function to the ~aov function:
lm.multifunc4 <- aov(mf_eff_1~TRAT2*DEPTH, asduffy_func)
...
##### Linear models_TOPSOIL
asduffy_func_top <- asduffy_func %>%
  filter(DEPTH=="topsoil")

mult_top <- lm(mf_eff_1~TRAT2, asduffy_func_top)

summary(mult_top)

##### Linear models_SUBSOIL
asduffy_func_sub <- asduffy_func %>%
  filter(DEPTH=="subsoil")

mult_sub <- lm(mf_eff_1~TRAT2, asduffy_func_sub)
...
RF_indicators <- as.data.frame(asduffy[, c(29:52, 59)])
str(RF_indicators)
#write.csv(asduffy_func[, c(11:16, 23)], "output/RF_functions.csv")
RF_functions <- as.data.frame(asduffy_func[, c(11:16, 23)])
...
#ggsave("output/RandomForest.png", height = 5, width = 6, dpi = 300)
```

```{r rfPermute}
#If we need statistical significance, we will use the rfPermute package (same base as   RandomForest)
set.seed(8)
rp <- rfPermute(mf_eff_1 ~ ., data=data, ntree = 300, num.rep = 1000, num.cores = 6)
...
#ggsave("output/rfPermute.png", height = 5, width = 6, dpi = 300)
```

```{r Correlations}
vars_cor <- RF_indicators
ggcorr(vars_cor, method = c("everything", "pearson"))
#ggsave("vars_cor.png")
```
...
depth_long_filtered <- depth_long %>%
  filter(variables %in% c("pH_difference_mean", "EC_difference_mean",
                          ...
                          "densidad_revisada_25_difference_mean"))
...
```{r}
#Calculation of the difference among depths
depth_dif <- soil_functions_clean %>%
  dplyr::select(TRAT2, DEPTH, all_of(func_variables)) %>%
  group_by(SITE,TRAT2) %>%
  dplyr::summarize(across(all_of(func_variables), 
                   ~ mean(.[DEPTH == "topsoil"]) - mean(.[DEPTH == "subsoil"]),
                   .names = "{.col}_difference"))
...
"pH", "EC", "densidad_revisada_25", "C_N_ratio",
              "A_phosphorus_ppm_105", "Phosphatase_activity_µmol_g_h", 
              "NH4_mgL_105","NO3_mgL_105", "Ergosterol_µg_g", 
              "Ntotal_g_100g_105", "Ctotal_g_100g_105", "Carbon_stock_Mg_ha",
              "Nitrogen_stock_Mg_ha", "SOM_per_corregido", "MBC_mg_kg_105",
