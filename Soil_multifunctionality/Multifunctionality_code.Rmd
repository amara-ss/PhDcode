...
pc1_loadings <- pca_sev$rotation[,1]
...
var <- dataset$C_N_ratio
var_log <- (log(dataset$C_N_ratio)+1)
...
##############################################################
mixed.var <- lmer(var_log ~ TRAT2*DEPTH + (1|SITE), data = dataset)
mixed.null <- lmer(var ~ (1|SITE), data = dataset)
anova(mixed.var)
summary(mixed.var)
?lmer
plot(mixed.var)
...
```{r Grouping into soil functions }
# 1) Define the variables
my_vars <- names(Firefungi_simple) %>% 
  intersect(c("pH", "EC", "Bulk_density_g_cm3", 
              "A_phosphorus_ppm_105", "Phosphatase_activity_µmol_g_h", 
              "NH4_mgL_105","NO3_mgL_105", "Ergosterol_µg_g", "Ntotal_g_100g_105", 
              "Ctotal_g_100g_105", "Carbon_stock_Mg_ha",
              "SOM_per_corregido", "MBC_mg_kg_105", "Nematode_g_105", 
              "Ca_g_kg", "K_g_kg","Mg_g_kg", "Na_g_kg", "Al_g_kg", "Fe_g_Kg",
              "Zn_mg_Kg_105", "Cu_mg_Kg_105"))
# 2) Normalization of variables
trial <- Firefungi_simple %>%
  mutate(across(all_of(my_vars),           
                ~ rescale(., to=c(0,1)),            
                .names = "{.col}_standardized"))
# 3) Create the 6 "soil function" categories with their mean and standard error: Soil_properties, Nutrient_pools, Soil_carbon, Soil_activity, Heavy_metals,Soil_biota:
soil_functions <- trial %>%
  dplyr::group_by(TRAT2, DEPTH) %>%
  dplyr::summarise(
    Soil_properties_mean = mean(c(pH_standardized, EC_standardized, Bulk_density_g_cm3_standardized), na.rm = TRUE),
    Soil_properties_se = sd(c(pH_standardized, EC_standardized, Bulk_density_g_cm3_standardized), na.rm = TRUE) / sqrt(n()),

    Nutrient_pools_mean = mean(c(A_phosphorus_ppm_105_standardized, NH4_mgL_105_standardized, NO3_mgL_105_standardized, Ntotal_g_100g_105_standardized, Ca_g_kg_standardized, K_g_kg_standardized, Mg_g_kg_standardized, Na_g_kg_standardized), na.rm = TRUE),
    Nutrient_pools_se = sd(c(A_phosphorus_ppm_105_standardized, NH4_mgL_105_standardized, NO3_mgL_105_standardized, Ntotal_g_100g_105_standardized, Ca_g_kg_standardized, K_g_kg_standardized, Mg_g_kg_standardized, Na_g_kg_standardized), na.rm = TRUE) / sqrt(n()),

    Soil_carbon_mean = mean(c(Ctotal_g_100g_105_standardized, SOM_per_corregido_standardized, Carbon_stock_Mg_ha_standardized), na.rm = TRUE),
    Soil_carbon_se = sd(c(Ctotal_g_100g_105_standardized, SOM_per_corregido_standardized, Carbon_stock_Mg_ha_standardized), na.rm = TRUE) / sqrt(n()),

    Soil_activity_mean = mean(c(Phosphatase_activity_µmol_g_h_standardized, MBC_mg_kg_105_standardized), na.rm = TRUE),
    Soil_activity_se = sd(c(Phosphatase_activity_µmol_g_h_standardized, MBC_mg_kg_105_standardized), na.rm = TRUE) / sqrt(n()),

    Heavy_metals_mean = mean(c(Al_g_kg_standardized, Cu_mg_Kg_105_standardized, Fe_g_Kg_standardized, Zn_mg_Kg_105_standardized), na.rm = TRUE),
    Heavy_metals_se = sd(c(Al_g_kg_standardized, Cu_mg_Kg_105_standardized, Fe_g_Kg_standardized, Zn_mg_Kg_105_standardized), na.rm = TRUE) / sqrt(n()),

    Soil_biota_mean = mean(c(Nematode_g_105_standardized, Ergosterol_µg_g_standardized), na.rm = TRUE),
    Soil_biota_se = sd(c(Nematode_g_105_standardized, Ergosterol_µg_g_standardized), na.rm = TRUE) / sqrt(n()),
    
    .groups = "drop"
  )
# 4) Define the functions:
EF <- names(soil_functions) %>% 
  intersect(c("Soil_properties_mean", "Nutrient_pools_mean", "Soil_carbon_mean", "Soil_activity_mean", "Heavy_metals_mean", "Soil_biota_mean"))
# 5) Reshape the data from wide to long format
soil_long <- soil_functions %>%
  pivot_longer(cols = c(Soil_properties_mean, 
                        Nutrient_pools_mean, 
                        Soil_carbon_mean, 
                        Soil_activity_mean, 
                        Heavy_metals_mean, 
  ...
  pivot_longer(cols = c(Soil_properties_se, 
                        Nutrient_pools_se, 
                        Soil_carbon_se, 
                        Soil_activity_se, 
                        Heavy_metals_se, 
  ...
  # Ensure that means and standard errors are correctly matched by trimming SE_function name
  filter(substr(Soil_function, 1, nchar(Soil_function)-5) == substr(SE_function, 1, nchar(SE_function)-3))

soil_long$Soil_function <- factor(soil_long$Soil_function, 
                           levels = c("Soil_properties_mean", "Nutrient_pools_mean",
                                      "Soil_carbon_mean", "Soil_activity_mean",
                                      "Soil_biota_mean", "Heavy_metals_mean"))
...
#ggsave("output/soil_functions.png")


##### Linear models
var_func <- soil_functions$Heavy_metals

lm_f <- aov(var_func ~ TRAT2, data = soil_functions)
summary(lm_f)
anova(lm_f)
Tuckeys<- TukeyHSD(lm_f)
print(Tuckeys)
...
```{r Multifunctionality index: dataset preparation}
multifunc_vars <- names(dataset) %>% 
...
Firefungi_simple <- dataset %>%
  dplyr::select(ID, TRAT2, DEPTH, all_of(multifunc_vars))
...
asduffy <- Firefungi_simple %>%
  dplyr::select(pc1, ID, TRAT2, DEPTH, all_of(multifunc_vars)) %>%
...
asduffy <- asduffy %>%
  cbind(getStdAndMeanFunctions(asduffy, multifunc_vars)) 

asduffy <-asduffy %>%
  getFuncsMaxed(multifunc_vars,
                threshmin=0.8, threshmax=0.8)

duffyAllVars.std <- paste0(multifunc_vars, ".std")
...
depth_dif <- dataset %>%
  dplyr::select(SITE, TRAT2, DEPTH, all_of(multifunc_vars)) %>%
  group_by(SITE, TRAT2) %>%
  dplyr::summarize(across(all_of(multifunc_vars), 
